...
tiPrim p = case p of
  ...
  (VariantElim label)    -> do
    ...
    b <- newTyVar 'b'
    r <- newTyVarWith Row (lacks label) 'r'
    return $ TFun (TRecord $ TRowExtend label a r)
                  (TFun (TFun a b) (TFun (TFun (TVariant r) b) b))
...
ppPrim (VariantInject l)  = "<" <> text l <> "=_>"
ppPrim (VariantEmbed l)   = "<" <> text l <> "|_>"
ppPrim (VariantElim l)    = "<" <> text l <> "==_?_:_>"
